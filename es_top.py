...
class ESTaskGetter:
    ...
    def __init__(self) -> None:
        ...
        self.show = "n"
        self.show_task_count = False  # else show runtime%*ntasks
    ...
    def set_urls(self, hosts: str) -> None:
        self.es = elasticsearch.Elasticsearch(
            hosts.split(","), opaque_id=self.opaque_id()
        )
    ...
    def _process_tasks(self, tad: dict[str, TaskDict]) -> None:
        ...
        for task_id, _td in tad["tasks"].items():
            ...
            elif not task_data["headers"].get("X-Opaque-Id") and self.show == "n":
                continue
...
class ESQueryGetter(ESTaskGetter):
    ...
    def task_descr(self, descr: str) -> str:
        if descr.startswith("index"):
            ...
            index = ""  # XXX extract index: indices[mc_search-*]
    ...
    def get_opaque_id(self, t: TaskDict) -> str:
        """
        get opaque_id header argument to Elasticsearch constructor
        (ie; generated by self.opaque_id for this code)
        from a TaskDict
        """
    ...
    def process_args(self) -> str:
        ...
        while n < argc:
            ...
            if arg == "--url":
                ...
                how = "once"
            elif arg[0] == "-" and len(arg) > 1 and arg[1].isalpha():
                for c in arg[1:]:
                    help = self.toggle(c)
                    if help:
                        self.usage(help)
